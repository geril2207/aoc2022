const input = await Deno.readTextFile('./input.txt').then((data) =>
  data.split('\r\n').map((item) => item.trim())
)
type MultiplierType = '+' | '*'
interface Monkey {
...
const superModulo = monkeys.reduce((prev, monkey) => monkey.testDivisible * prev, 1)
...
function increaseByMultiplier(
  ...
  if (type === '*') return item * multiplier
  else return item + multiplier
}
...
function oneMonkeyRound(monkey: Monkey) {
  ...
  while (monkeyItems.length) {
    ...
    const newItem = increaseByMultiplier(item, monkey.worryMultiplier, monkey.multiplierType) % superModulo
